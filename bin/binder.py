...
def scan_file(
    ...
    with open(path) as sfile:
        ...
        for line in sfile:
            ...
            # class
            elif line.lstrip().startswith("class ") and not line.endswith(";"):
                class_name = (
            ...
            # @enum
            elif ENUM in line and "enum" in parts:
                headers.add(basename)
            ...
            # @enum
            ...
            elif STRUCT in line and "struct" in parts:
                if "typedef" in parts:
                    print(f"typedef not supported on line {line_num}: {line}")
                    continue
                headers.add(basename)
                a = line.split("|", 1)
                ctr = a[1] if len(a) > 1 else ""
                a = line.strip().split("//", 1)
                ...
                structs.fields[struct_name].append(
                    [name, f"{class_name}::{struct_name}::{name}"]
                )
